 ...
 */
    ...
    (function (window, $, undefined) {		
		var community = window.community = window.community || { ui : {}, model : {} , data : { model : {}}},
		extend = $.extend,
		each = $.each,
		isArray = $.isArray,
		proxy = $.proxy,
		...
		var Setup = kendo.Observable.extend({		
			...
			authenticate : function( callbackHandler ){
				...
				if(defined(features.accounts)){
					ajax( features.accounts.url || CONTEXT_PATH + '/data/api/v1/users/me.json' , {
			...
			_features: function(){
				...
				/** START UNIFY */
				if(defined(features.unify.header)){
					// setting unify header component.
					var __renderTo =  features.unify.header.renderTo || $('#js-header') ;
					$.HSCore.components.HSHeader.init(__renderTo);	
					console.log("unify header initialized.");
				}
				if(defined(features.unify.hamburgers)){
					// setting unify header component.
					var __renderTo =  features.unify.hamburgers.renderTo || '.hamburger' ;
					$.HSCore.helpers.HSHamburgers.init(__renderTo); 
					console.log("unify hamburgers initialized.");
				}
				if(defined(features.unify.dropdown)){ 
					// setting unify header component.
					var __renderTo =  features.unify.dropdown.renderTo || $('[data-dropdown-target]') ;
					$.HSCore.components.HSDropdown.init( __renderTo  );
					console.log("unify dropdown initialized.");
				}
				/** END UNIFY */
				
				if(features.tooltip){
					// Botostrap Tooltips enable.	
