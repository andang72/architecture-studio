...
public class JdbcMenuDao extends ExtendedJdbcDaoSupport implements MenuDao { 
	...
	public void delete(Menu menu) {
		if( menu.getMenuId() > 0 ) {
			getExtendedJdbcTemplate().update(getBoundSql("COMMUNITY_UI.DELETE_MENU_BY_ID").getSql(), new SqlParameterValue(Types.NUMERIC, menu.getMenuId() ));
			if( menu.getProperties().size() > 0 )
				deleteMenuProperties(menu.getMenuId()); 
			List<Long> list = getMenuItemIds(menu.getMenuId());
			for(Long id : list ) {
				getExtendedJdbcTemplate().update(getBoundSql("COMMUNITY_UI.DELETE_MENU_ITEM").getSql(), new SqlParameterValue(Types.NUMERIC, id ));
				deleteMenuItemProperties(id);
			}
		}
